...
--
-- Gép: localhost:8889
-- Létrehozás ideje: 2026. Feb 16. 20:02
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_screening` (IN `p_film_id` INT, IN `p_start` TIME, IN `p_screening_date` DATE)   BEGIN
  INSERT INTO screening (film_id, start_time, screening_date, created_at, updated_at)
  VALUES (p_film_id, p_start, p_screening_date, NOW(), NOW());
END$$
