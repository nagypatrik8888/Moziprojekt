...
--
-- Gép: localhost:8889
-- Létrehozás ideje: 2026. Feb 23. 08:52
...
ALTER TABLE `ticket_order_seats`
  ADD PRIMARY KEY (`ticket_order_id`,`seat_id`),
  ADD UNIQUE KEY `uq_screening_seat` (`screening_id`,`seat_id`),
  ADD KEY `idx_tos_seat` (`seat_id`),
  ADD KEY `fk_tos_price` (`price_id`);
...
--
ALTER TABLE `ticket_order_seats`
  ADD CONSTRAINT `fk_tos_price` FOREIGN KEY (`price_id`) REFERENCES `prices` (`id`),
  ADD CONSTRAINT `fk_tos_screening` FOREIGN KEY (`screening_id`) REFERENCES `screening` (`id`),
